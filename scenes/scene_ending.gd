...
func _ready() -> void:
	...
	for node: CanvasItem in image_container.get_children():
		node.modulate.a = 0
...
func next_image() -> void:
	print("Next Image")
	tween = create_tween()
	var node: CanvasItem = image_container.get_child(image_index)
	text_label.visible_ratio = 0
	text_label.text = tr(texts[image_index])
	tween.tween_property(node, "modulate", Color.WHITE, 0.4)
	tween.tween_property(text_label, "visible_ratio", 1, 2.0)
	image_index += 1
